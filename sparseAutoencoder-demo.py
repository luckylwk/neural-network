...
def loadDataset(num_patches, patch_side):
    ...
    images = images['IMAGES'] # Size is 512-by-512-by-10. Where 512x512 is the image dimension.
    

    """ Initialize dataset as array of zeros """
    
    dataset = numpy.zeros((patch_side*patch_side, num_patches)) # (8x8)64-by-10000
    
    """ Initialize random numbers for random sampling of images
        There are 10 images of size 512 X 512 """
    
    rand = numpy.random.RandomState(int(time.time()))
    image_indices = rand.randint(512 - patch_side, size = (num_patches, 2)) # 10000-by-2 array of random numbers between 0-504(512-8)
    image_number  = rand.randint(10, size = num_patches) # 10000-by-1 with integer between 0-9
    ...
    for i in xrange(num_patches):
        ...
        patch = images[index1:index1+patch_side, index2:index2+patch_side, index3] # extract 8-by-8 patches from random image.
